...
describe("Ski Lift", function () {
  ...
  it('should seat skiers', function() {
    ...
    lift.admitSkier('Pam', true);//name & hasTicket
    lift.admitSkier('Jeff', true);

// console.log(lift.skiers.length);
  ...
  it('should only seat skiers if there are enough spots', function() {
    var lift = new Lift(2);//2 seats available
...
// console.log(lift.skiers.length);
  ...
  it('should only seat skiers that have lift tickets', function () {
    var lift = new Lift(1);//one seat available

    var admit = lift.admitSkier('Hannah', false);//did not have a ticket
    lift.admitSkier('Scott', true);//did have a ticket
  ...
  it('should lower the safety bar once all skier positions are filled', function() {
    var lift = new Lift(4);//4 seats available

    lift.admitSkier('Hannah', false);//no ticket
    lift.admitSkier('Scott', true);
...
// console.log('skiers -', lift.skiers.length);
  ...
  it('should not lower the safety bar until all skier positions are filled', function () {
    ...
    lift1.admitSkier('Hannah', false);//does not admit
    lift1.admitSkier('Scott', false);//does not admit
    lift1.admitSkier('Will', true);
    lift1.admitSkier('Eric', true);
    lift1.admitSkier('Leta', true);

    lift2.admitSkier('Hannah', true);
    lift2.admitSkier('Kayla', false);//does not admit
